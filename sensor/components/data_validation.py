...
class DataValidation:
    """
    Description: This class performs validation of data in the dataset and prepares Data drift report.
    ...
    def drop_unnecessary_columns(self, df:pd.DataFrame, name:str)->pd.DataFrame:
        ...
        try:
            ...
            if schema_drop_columns:
                ...
                logging.info("Columns [{0}] are dropped from dataset [{1}].".format(drop_column_names, name))
                return df
            logging.info("No column to drop in the dataset [{0}].".format(name))
            return df
        except Exception as e:
    ...
    def validate_number_of_columns(self, df:pd.DataFrame, name:str)->bool:
        """
        Description: This functions checks if all the standard columns required are present in the dataset.
    ...
    def is_numerical_column_exists(self, df:pd.DataFrame, name:str)->bool:
        """
        Description: This functions checks if all the standard numerical columns present in the dataset.\
    ...
    def check_data_drift(self, base_df:pd.DataFrame, current_df:pd.DataFrame 
        ...
        """
        Description: This function prepares data drift report for the given two dataframe.
    ...
    def read_data(file_path:str)->pd.DataFrame:
        """
        Description: This function read the dataset from the file path and gives a pandas dataframe.
        ...
        """
        try:
            logging.info("Reading the [{0}] file for validation.".format(
    ...
    def initiate_data_validation(self,)->DataValidationArtifact:
        try:
            ...
            # drop the schema_drop_columns
            train_df = self.drop_unnecessary_columns(df=train_df, name="Train data")
            test_df = self.drop_unnecessary_columns(df=test_df, name="Test data")

            # validate number of columns
            status = self.validate_number_of_columns(df=train_df, name="Train data")
            if not status:
                error_message=f"{error_message}Train dataframe doesnot contain all the standard columns.\n"
            status = self.validate_number_of_columns(df=test_df, name="Test data")
            if not status:
                error_message=f"{error_message}Test dataframe doesnot contain all the standard columns.\n"

            # validate numerical columns
            status = self.is_numerical_column_exists(df=train_df,name="Train data")
            if not status:
                error_message = f"{error_message}Train datframe doesnot contain all the standard numerical columns.\n"
            status = self.is_numerical_column_exists(df=test_df, name="Test data")
            if not status:
                error_message = f"{error_message}Test dataframe doesnot contain all the standard numerical columns.\n"
